...
app.get("/api/stats", async (_req, res) => {
  try {
    const [nodesByChannel, edgesByType, topConnected, recentNodes, orphans] = await Promise.all([
      ...
      graph.query(`MATCH (n) WHERE NOT (n)-[]-() AND n.\`${NAME_PROP}\` IS NOT NULL RETURN n.\`${NAME_PROP}\` AS name, n.channel AS channel`),
    ]);
    res.json({
      ...
      orphan_nodes: (orphans.data || []).map(r => ({ name: r.name, channel: r.channel })),
    });
  } catch (err) {
